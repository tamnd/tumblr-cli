...
describes what users get. Pick a format with `-o`, or let tumblr choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
tumblr <command> -o table   # aligned columns for reading
tumblr <command> -o jsonl   # one JSON object per line, for piping
tumblr <command> -o json    # a single JSON array
tumblr <command> -o csv     # spreadsheet friendly
tumblr <command> -o tsv     # tab-separated
tumblr <command> -o url     # just the URL column
tumblr <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
tumblr <command> --fields id,title,url
```
...
```bash
tumblr <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
tumblr <command>            # a table, because this is a terminal
tumblr <command> | wc -l    # JSONL, because this is a pipe
```
